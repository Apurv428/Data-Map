...
const Page2 = () => {
  ...
  const checkError = async () => {
    console.log("Q =", q);
    var error = await axios.get(`http://127.0.0.1:5000/is_in/${q}`);
  ...
  const fetchMovies = async () => {
    const { data } = await axios.get(
      `https://api.themoviedb.org/3/search/movie?api_key=793f2a03af5ed1e5355999fdee3ca252&language=en-US&query=${q}&page=1&include_adult=false`
    );
  ...
  const fetchVideo = async () => {
    ...
    const video = await axios.get(
      `https://api.themoviedb.org/3/movie/${id}/videos?api_key=&language=${lang}`
    );
