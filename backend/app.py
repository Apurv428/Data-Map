...
def similar_to(user_movie):
    ...
    similar_movies = cursor.execute(
        "SELECT * FROM Movies WHERE LOWER(title)=?", (movie, ))
    ...
    try:
        ...
        for rank, mov in res.items():
            ...
            val = val - 15
        final["nodes"].append(
            {"id": similar[0][1], "color": "rgba(165, 85, 253, 1)"})
    except:
...
def is_in(user_movie):
    movie = user_movie.lower()  # converting user enetered string to lowercase
    cursor = conn.cursor()
    is_movie = cursor.execute(
        "SELECT COUNT(title) FROM Movies WHERE LOWER(title)=?", (movie,))
